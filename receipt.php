...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT u.user_name, u.user_gmail, u.user_phone_number, u.user_address,
               b.event_type, b.event_address, b.payment_status,
               p.payment_method, p.payment_amount, p.transaction_id
        FROM users u
    ...
    <style>
        ...
        .header { display: flex; align-items: center; background: linear-gradient(to bottom, #00576d, #00768c); color: white; padding: 15px; border-radius: 5px; margin-bottom: 20px; }
        .header-text { flex-grow: 1; text-align: center; }
        .header-text h1 { font-size: 24px; margin: 0; font-weight: 700; }
        .header-text h2 { font-size: 16px; margin: 5px 0 0; font-weight: 400; }
    ...
    if (!empty($data['payment_method'])) {
        ...
        $html .= '<tr><th>Payment Amount</th><td>$' . htmlspecialchars(number_format($data['payment_amount'], 2)) . '</td></tr>';
